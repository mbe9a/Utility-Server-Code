...
class Scan(object):
    def __init__(self, filename):
        ...
        self.ht = np.rot90(np.array(self.data['wave']['wData'])[:, :, 0])
        self.amp = np.rot90(np.array(self.data['wave']['wData'])[:, :, 1])
        self.ph = np.rot90(np.array(self.data['wave']['wData'])[:, :, 2])
        self.z = np.rot90(np.array(self.data['wave']['wData'])[:, :, 3])

    def height_retrace(self):
        plt.figure()
        plt.title('Height Retrace', fontsize=14)
        plt.imshow(self.ht * 1e9, cmap='copper', interpolation='nearest',
    ...
    def amplitude_retrace(self):
        ...
        plt.imshow(self.amp * 1e9, cmap='copper', interpolation='nearest',
                   extent=[0, self.scan_size, 0, self.scan_size])
        plt.colorbar().set_label('$nm$', fontsize=14, rotation=0, horizontalalignment='left')
        plt.xlabel('$\mu m$', fontsize=14)
        plt.ylabel('$\mu m$', fontsize=14, rotation=0, labelpad=20)
    ...
    def phase_retrace(self):
        ...
        plt.imshow(self.ph, cmap='copper', interpolation='nearest',
                   extent=[0, self.scan_size, 0, self.scan_size])
    ...
    def zsensor_retrace(self):
        ...
        plt.imshow(self.z * 1e9, cmap='copper', interpolation='nearest',
                   extent=[0, self.scan_size, 0, self.scan_size])
        plt.colorbar().set_label('$nm$', fontsize=14, rotation=0, horizontalalignment='left')
        plt.xlabel('$\mu m$', fontsize=14)
        plt.ylabel('$\mu m$', fontsize=14, rotation=0, labelpad=20)
